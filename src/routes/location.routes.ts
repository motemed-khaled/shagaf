...
router
  .route('/')
  .post(isauthenticated, val.Create, handler.createLocationHandler)
  .get(handler.getLocationsHandler);

router
  .route('/:locationId')
  .patch(isauthenticated, val.update, handler.updateLocationHandler)
