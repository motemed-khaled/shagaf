...
router.get('/user/book' , isauthenticated ,val.getAllBookingVal , globalPaginationMiddleware , handler.getUserBookingHandler);
router.route('/book')
  .post(isauthenticated , val.memberBookVal , handler.bookMemberHandler)
  .get(isauthenticated , val.getAllBookingVal , globalPaginationMiddleware , handler.getMemberBookingsPagination , handler.getAllBookingHandler);
