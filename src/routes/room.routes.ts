...
router.route('/').post(isauthenticated ,
  globalUploadMiddleware(FOLDERS.room, {
    ...
    fileTypes: ['image'],
  }).fields([{ name: 'cover', maxCount: 1 } , { name: 'attachments', maxCount: 10 }]),
  val.createRoom ,
  checkRequiredFields({ fields: ['cover' , 'attachments'] }),
  handlers.createRoomHandler)
  .get(val.getAll  , globalPaginationMiddleware, handlers.getRoomsPagination , handlers.getRoomsHandler);
...
router.route('/:roomId')
  .patch(isauthenticated ,
    ...
    val.updateRoom , handlers.updateRoomHandler)
...
router.route('/:roomId/amenities/:amenitiesId')
  .patch(
    isauthenticated ,
    globalUploadMiddleware(FOLDERS.room, {
      maxSize: 50 * 1024 * 1024,
