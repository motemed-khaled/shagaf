...
router.route('/')
  .post(isauthenticated , globalUploadMiddleware(FOLDERS.adds, {
    maxSize: 50 * 1024 * 1024,
    fileTypes: ['image'],
  }).fields([{name:'cover' , maxCount:1}]),checkRequiredFields({ fields: ['cover'] }) , handler.createAddHandler)
  .get(handler.getAddsHandler);

router.route('/:addId')
  .patch(isauthenticated , globalUploadMiddleware(FOLDERS.adds, {
    maxSize: 50 * 1024 * 1024,
    fileTypes: ['image'],
  }).fields([{name:'cover' , maxCount:1}]) , val.getAddVal,checkRequiredFields({ fields: ['cover'] }) , handler.updateAddHandler)
