...
router.route('/')
  .post(isauthenticated , globalUploadMiddleware(FOLDERS.category, {
    maxSize: 50 * 1024 * 1024,
    fileTypes: ['image'],
  }).fields([{name:'cover' , maxCount:1}]) , val.createCategoryVal , checkRequiredFields({ fields: ['cover'] }) , handler.createCategoryHandler)
  .get(globalPaginationMiddleware , handler.getCategoriesHandler);
  

router.route('/:categoryId')
  .patch(isauthenticated ,globalUploadMiddleware(FOLDERS.category, {
    maxSize: 50 * 1024 * 1024,
    fileTypes: ['image'],
  }).fields([{name:'cover' , maxCount:1}]) , val.updateCategoryVal , handler.updateCategoryHandler )
