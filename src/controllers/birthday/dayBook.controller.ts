...
export const dayBookHandler: DayBookHandler = async (req, res, next) => {
  ...
  if (req.body.user) {
    ...
    if (!user) return next(new NotFoundError(`user ${req.body.user} not found`));
  }

  const products = await BirthDay.countDocuments({
    _id: req.body.products.map((el) => el.product),
  });

  if (products != req.body.products.length) return next(new BadRequestError('invalid products'));
