...
export const createPaymentHandler:CreatePaymentHandler = async (req,res,next)=>{
  let book;
  if (req.body.type == PaymentType.dayBook) 
    book = await DayBook.findById(req.body.bookId);
  if (req.body.type == PaymentType.eventBook) 
    book = await EventBook.findById(req.body.bookId);
  if (req.body.type == PaymentType.memberBook) 
    book = await MemberBooking.findById(req.body.bookId);
  if (req.body.type == PaymentType.roomBooking) 
    book = await RoomBooking.findById(req.body.bookId);

  if (!book) 
    return next(new NotFoundError('book not found'));

  try {
    const paymentResponse = await KashierPayment.createPayment(book.totalPrice , 'EGP' , book._id);
