...
export const getLoggedUserOffersHandler:GetLoggedUserOfferHandler = async (req,res)=>{
  const offers = await Offer.find({users:{$in:[new mongoose.Types.ObjectId(req.loggedUser?.id)]}})
    .limit(req.pagination.limit).skip(req.pagination.skip);

  const resultCount = await Offer.find({users:{$in:[new mongoose.Types.ObjectId(req.loggedUser?.id)]}}).countDocuments();
