...
> = async (req, res, next) => {
  ...
  if (req.query.countMin) {
    req.pagination.filter.count = { ...req.pagination.filter.count, $gte: Number(req.query.countMin) };
  }
  if (req.query.countMax) {
    req.pagination.filter.count = { ...req.pagination.filter.count, $lte: Number(req.query.countMax) };
  }
...
export const getProductsHandler:GetProductsHandler = async (req,res)=>{
  const products = await Product.find(req.pagination.filter).limit(req.pagination.limit).skip(req.pagination.skip);
