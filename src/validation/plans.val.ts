...
export const updatePlanVal = [
  ...
  body('type')
    .optional()
    .isIn(Object.values(PlanTypes))
    .custom((val, { req }) => {
      const requestBodyKeys = Object.keys(req.body);

      if (val === PlanTypes.shared) {
        if (
          requestBodyKeys.some((key) => key.startsWith('private') || key.startsWith('birthDay'))
        ) {
        ...
        return [
          body('shared.hourOne')
            ...
            .bail()
            .optional()
            .withMessage('hourOne must be a number'),
          body('shared.hourTwo')
            .isInt({ min: 1 })
            .bail()
            .optional()
            .withMessage('hourTwo must be a number'),
          body('shared.hourThree')
            .isInt({ min: 1 })
            .bail()
            .optional()
            .withMessage('hourThree must be a number'),
          body('shared.hourFour')
            .isInt({ min: 1 })
            .bail()
            .optional()
            .withMessage('hourFour must be a number'),
        ];
      }
...
export const getPlan = [param('planId').isMongoId().bail(), validationMiddleware];
