...
export const createOfferVal = [
  body('discount')
    .isInt({ min: 1, max: 100 })
    .withMessage('Discount must be an integer between 1 and 100'),
  body('from')
    .optional()
    .isISO8601()
    .toDate()
    .custom((val, { req }) => {
      if (!req.body.to) throw new Error('To date is required');
      const from = new Date(val);
      const now = new Date();
      if (from < now) throw new Error('From date cannot be in the past');
      return true;
    })
  ...
  body('to')
    ...
    .toDate()
    .custom((val, { req }) => {
      if (!req.body.from) throw new Error('From date is required');
