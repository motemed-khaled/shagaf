...
export const signUpVal = [
  body('username')
    ...
    .trim()
    .withMessage('Username must be between 3 and 35 characters'),
  body('password')
    .isStrongPassword({
      minLength: 8,
      minLowercase: 1,
...
export const createUserVal = [
  ...
  body('userType')
    .optional()
    .isIn(['manager', 'stuff', 'user'])
    .withMessage('user must by one of this manager stuff user '),
  validationMiddleware,
];
...
export const signInVal = [
  // Corrected the name from `sigInVal` to `signInVal`
  body('email').isEmail().withMessage('Invalid email address'),
