...
export const createDayVal = [
  ...
  body('type')
    .isIn(['cake', 'decoration', 'session'])
    .withMessage('Type must be one of "cake", "decoration", or "session"'),
...
export const getDaysVal = [
  query('type')
    ...
    .isString()
    .isIn(['cake', 'decoration', 'session'])
    .withMessage('Type must be either "cake", "decoration", or "session"'),
...
export const getAllBookingVal = [
  ...
  query('status')
    ...
    .withMessage('status must be value of rejected , accepted , pending '),
  validationMiddleware,
];
